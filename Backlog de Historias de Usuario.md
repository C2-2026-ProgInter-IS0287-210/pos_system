...
**Criterios de Aceptación:**
- [x] Dado un descuento porcentual o monto fijo válido, cuando se aplica a una `LineaVenta`, entonces el subtotal debe recalcularse correctamente.
- [x] Dada una venta previamente en estado `PAGADA`, cuando se aprueba una devolución, entonces se genera la instancia `Devolucion` y se invoca la restitución del total/parcial.
...
**Descripción Técnica:**
Desarrollo de `CargarConfiguracionCasoUso` (Entidad `ConfiguracionGlobal` y sus enumerados `Idioma` y `Moneda`, `Sucursal`) y `GestionarTurnoCasoUso` (Entidad `Turno`, `Asistencia`).
...
**Descripción Técnica:**
Coordinado por `ControladorCaja` invocando `AbrirCajaCasoUso`, `CerrarCajaCasoUso` y `RegistrarMovimientoCajaCasoUso`. Se fundamenta en `ServicioCaja` manejando la `CajaPrincipal`, `MovimientoCaja`, `CierreCaja` (generando un `Reporte`), apoyado por `RepositorioCajaImpl`.
...
**Descripción Técnica:**
Construcción de los casos de uso `GenerarInformeVentasCasoUso` y `GenerarDashboardCasoUso` llamando al `ServicioInformes`. Las entidades son `Informe`, `Reporte` y `Dashboard`. Conexión con `RepositorioInformesImpl` y `RepositorioVentasImpl`.
...
**Descripción Técnica:**
Responsabilidad sobre el contenedor padre `VentanaPrincipal` (que orquesta los demás paneles inyectando controladores). A nivel backend, desarrollo de `ImportarProductosCasoUso` implementando la interfaz `ServicioImportacion` a través de clases de infraestructura como `ImportadorCSV` e `ImportadorDatos`.
